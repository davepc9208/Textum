...
export default function Navbar() {
  ...
  // Ruta con el ancla: desde cualquier página enlaza a la landing en la sección.
  const anchorHref = (href: string) => localizedPath('/' + href, lang);
  const contactoHref = localizedPath('/#contacto', lang);
  ...
  const renderNavItem = (item: NavItem, index: number) => {
    ...
    const href = anchorHref(anchor.href);
    return (
      <li key={anchor.href}>
        <Link
          to={href}
    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      ...
      <div id="navbar-mobile-menu" className={`lg:hidden overflow-hidden transition-all duration-400 ${open ? 'max-h-[52rem] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="glass-navy border-t border-gold/20 px-6 py-5 flex flex-col gap-1">
          ...
          <Link to={contactoHref} onClick={closeMenu}

...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      ...
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-3">
          <button onClick={() => setLang(nextLang)}
            ...
            <span className="absolute top-1 bottom-1 w-8 rounded-full bg-gold transition-transform duration-300 ease-out" aria-hidden="true"
              style={{ transform: lang === 'es' ? 'translateX(0%)' : 'translateX(100%)' }} />
            <span className={`relative z-10 w-8 text-center py-1 transition-colors duration-200 ${lang === 'es' ? 'text-navy font-semibold' : 'text-gold/60'}`}
              aria-current={lang === 'es' ? 'true' : undefined}>ES</span>
            <span className={`relative z-10 w-8 text-center py-1 transition-colors duration-200 ${lang === 'en' ? 'text-navy font-semibold' : 'text-gold/60'}`}
              aria-current={lang === 'en' ? 'true' : undefined}>EN</span>
          </button>
          ...
          <a href={isHome ? '#contacto' : '/#contacto'} onClick={handleContactoCta}
      ...
      <div className={`lg:hidden overflow-hidden transition-all duration-400 ${open ? 'max-h-[52rem] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="glass-navy border-t border-gold/20 px-6 py-5 flex flex-col gap-1">
          ...
          <button onClick={() => { setLang(nextLang); setOpen(false); }}
          ...
          <a href={isHome ? '#contacto' : '/#contacto'} onClick={(e) => { handleContactoCta(e); setOpen(false); }}
